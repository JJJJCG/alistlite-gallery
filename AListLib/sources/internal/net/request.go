...
func (d Downloader) Download(ctx context.Context, p *HttpRequestParams) (readCloser io.ReadCloser, err error) {
	...
	copyHttpRequestParams(&finalP, p)
...
func (d *downloader) getParamsFromChunk(ch *chunk) *HttpRequestParams {
	...
	copyHttpRequestParams(&params, d.params)
...
// copyHttpRequestParams 语义等价于原先使用的 awsutil.Copy（来自 aws-sdk-go）。
// 为移除 aws-sdk-go 这一超重依赖，此处改为本地实现：
// 值字段直接赋值，HeaderRef 单独做一层浅拷贝，避免两个 params 共享同一个 header map。
func copyHttpRequestParams(dst, src *HttpRequestParams) {
	*dst = *src
	if src.HeaderRef != nil {
		h := make(http.Header, len(src.HeaderRef))
